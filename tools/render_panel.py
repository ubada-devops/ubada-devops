...
def render_panel():
    ...
    for idx, (k, v) in enumerate(ROWS):
        ...
        row_style = "" if is_preview else f'style="animation: fadeIn 0.5s forwards {delay:.1f}s;"'
        
        svg.append(f'  <g class="row" {row_style}>')
    ...
    if not is_preview:
        svg.append('  <style> @keyframes fadeIn { to { opacity: 1; } } </style>')
